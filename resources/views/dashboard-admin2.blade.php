                            ...
                            @foreach ($pendaftarans as $index => $pendaftaran)
                                <tr>
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <a href="{{ route('admin.users.edit', $pendaftaran->user->user_id) }}"
                                            class="text-indigo-600 hover:text-indigo-900">{{ $pendaftaran->user->name }}</a>
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <a href="{{ route('admin.Pesertas.index', ['kelompok' => $pendaftaran->kelompok->nama_kelompok]) }}"
                                            class="text-indigo-600 hover:text-indigo-900">{{ $pendaftaran->kelompok->nama_kelompok }}</a>
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        @php
                                            $judul_proposal = str_replace(
                                                ' ',
                                                ' ',
                                                $pendaftaran->proposal->judul_proposal,
                                            );
                                        @endphp
                                        <a href="{{ route('admin.proposals.index', ['search' => $judul_proposal]) }}"
                                            class="text-indigo-600 hover:text-indigo-900">{{ $pendaftaran->proposal->judul_proposal }}</a>
                                    </td>
