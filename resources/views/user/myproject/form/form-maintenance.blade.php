    ...
    <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
        <div class="p-6 bg-white border border-gray-200">
            @foreach ($pendaftarans as $daftar)
                ...
                <form action="{{ route('maintenance.input', ['proposal_id' => $daftar->proposal->proposal_id]) }}" method="post"
                    enctype="multipart/form-data">
                    @csrf
                    <div>
                        <x-label for="deskripsi_maintenance" :value="__('Deskripsi Maintenance')"></x-label>
                        @if ($daftar->proposal->deskripsi_maintenance == null)
                            <input id="deskripsi_maintenance" type="hidden" name="deskripsi_maintenance">
                            <trix-editor input="deskripsi_maintenance"></trix-editor>
                        @else
                            <input id="deskripsi_maintenance" type="hidden" name="deskripsi_maintenance">
                            <trix-editor input="deskripsi_maintenance"></trix-editor>
                        @endif
                    ...
                    <div class="col-span-2 mt-3">
                        <x-label for="file_maintenance" :value="__('Maintenance File (PDF)')"></x-label>
                        ...
                        @else
                            <x-input id="file_maintenance"
                                class=" block w-fit text-sm text-gray-900 border border-gray-300 rounded-lg cursor-pointer bg-gray-50 dark:text-gray-400 focus:outline-none p-3"
                                type="file" name="file_maintenance" value="{{ $daftar->proposal->file_maintenance }}"></x-input>
