    ...
    <div class="mb-4">
        <form action="{{ route('admin.Pesertas.index') }}" method="GET">
            <div class="flex items-center">
                <input type="text" name="search" class="w-64 rounded-md border-gray-300 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50" placeholder="Search by name, NPSN, or NIM..." value="{{ request('search') }}">
                ...
                <table class="min-w-full divide-y divide-gray-200 bg-white text-sm">
                    <thead class="ltr:text-left rtl:text-right">
                        <tr>
                            ...
                            <th scope="col" class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Nomor WA</th>
                            <th scope="col" class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Kemampuan Deskripsi</th>
                        ...
                        @foreach($pesertas as $index => $peserta)
                            <tr>
                                ...
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <a href="{{ route('admin.Pesertas.edit', $peserta->id) }}" class="text-indigo-600 hover:text-indigo-900">Edit</a>
                                    <form action="{{ route('admin.Pesertas.destroy', $peserta->id) }}" method="POST" class="inline">
                                        @csrf
                                        @method('DELETE')
